 ...
 public:
  ...
  void send() override {
    try {
      ...
      // Unthrottled log so we can validate one command per cmd_vel (was RCUTILS_LOG_INFO_THROTTLE)
      RCUTILS_LOG_INFO(
          "CmdDoBufferedM1M2DriveSpeedAccelDistance::send starting at %ld ms (accel: %d, m1Speed: "
      ...
      roboclaw_.appendToWriteLog(
          ...
          m2_max_distance_quad_pulses_);
      roboclaw_.writeN2(
          true, 23, roboclaw_.portAddress_, RoboClaw::MIXEDSPEEDACCELDIST,
      ...
      auto send_duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(send_end - send_start).count();
      RCUTILS_LOG_INFO(
          "CmdDoBufferedM1M2DriveSpeedAccelDistance::send completed at %ld ms (duration: %ld ms)",
          std::chrono::duration_cast<std::chrono::milliseconds>(send_end.time_since_epoch())
              .count(),
          send_duration);
    } catch (RoboClaw::TRoboClawException* e) {
      RCUTILS_LOG_ERROR(
          "[RoboClaw::CmdDoBufferedM1M2DriveSpeedAccelDistance] RoboClaw exception: %s", e->what());
    } catch (std::exception& e) {
      RCUTILS_LOG_ERROR(
          "[RoboClaw::CmdDoBufferedM1M2DriveSpeedAccelDistance] Standard exception: %s", e.what());
    } catch (...) {
      RCUTILS_LOG_ERROR(
