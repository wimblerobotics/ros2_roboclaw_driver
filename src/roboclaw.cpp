...
void RoboClaw::doMixedSpeedAccelDist(uint32_t accel_quad_pulses_per_second,
  ...
  CmdDoBufferedM1M2DriveSpeedAccelDistance command(*this, accel_quad_pulses_per_second,
...
uint32_t RoboClaw::getErrorStatus() {
  uint32_t status = 0;
  try {
    CmdReadStatus cmd(*this, status);
    cmd.execute();
    g_sensor_value_group_.error_status = status;
    g_sensor_value_group_.error_string = getErrorString(status);
  } catch (...) {
...
std::string RoboClaw::getErrorString() {
  return getErrorString(getErrorStatus());
}
...
float RoboClaw::getLogicBatteryLevel() {
  float v = 0.0f;
  try {
    CmdReadLogicBatteryVoltage c(*this, v);
    c.execute();
    g_sensor_value_group_.logic_battery_level = v;
  } catch (...) {
  }
...
float RoboClaw::getMainBatteryLevel() {
  float v = 0.0f;
  try {
    CmdReadMainBatteryVoltage c(*this, v);
    c.execute();
    g_sensor_value_group_.main_battery_level = v;
  } catch (...) {
  }
...
RoboClaw::TMotorCurrents RoboClaw::getMotorCurrents() {
  TMotorCurrents currents{};
  try {
    CmdReadMotorCurrents c(*this, currents);
    c.execute();
    g_sensor_value_group_.motor_currents = currents;
  } catch (...) {
  }
...
float RoboClaw::getTemperature() {
  float t = 0.0f;
  try {
    CmdReadTemperature c(*this, t);
    c.execute();
    g_sensor_value_group_.temperature = t;
  } catch (...) {
  }
...
int32_t RoboClaw::getVelocity(kMotor motor) {
  int32_t speed = 0;
  try {
    CmdReadEncoderSpeed c(*this, motor, speed);
    c.execute();
    if (motor == kM1)
      g_sensor_value_group_.m1_velocity = speed;
    else
      g_sensor_value_group_.m2_velocity = speed;
  } catch (...) {
  }
  return (motor == kM1) ? g_sensor_value_group_.m1_velocity : g_sensor_value_group_.m2_velocity;
}
...
uint32_t RoboClaw::getM1Encoder() {
  EncodeResult r{};
  try {
    CmdReadEncoder c(*this, kM1, r);
    c.execute();
    g_sensor_value_group_.m1_encoder_command_result = r;
  } catch (...) {
  }
...
uint32_t RoboClaw::getM2Encoder() {
  EncodeResult r{};
  try {
    CmdReadEncoder c(*this, kM2, r);
    c.execute();
    g_sensor_value_group_.m2_encoder_command_result = r;
  } catch (...) {
  }
...
void RoboClaw::openPort() {
  ...
  switch (baud_rate_) {
    ...
    case 115200:
      baud = B115200;
      break;
    case 2000000:  // 2 Mbps high-speed option
#ifdef B2000000
      baud = B2000000;
      break;
#else
      RCUTILS_LOG_ERROR("[RoboClaw::openPort] 2,000,000 baud not supported on this platform");
      throw new TRoboClawException("[RoboClaw::openPort] Unsupported baud rate 2000000");
#endif
    default:
