...
namespace roboclaw_driver {
    void SafetySupervisor::reset() {
        ...
        runaway_start_ = 0; stall_start_ = 0; // stall retained for backward compat but unused
    }
    SafetySupervisor::Result SafetySupervisor::evaluate(const SafetySample& s) {
        ...
        // Stall estop removed per user request: buffered distance-limited drive commands already prevent run-on motion when cmd_vel gaps occur.
        return r;
    }
} // namespace roboclaw_driver
