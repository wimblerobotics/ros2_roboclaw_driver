...
void MotorDriver::initializeParameters(rclcpp::Node& node) {
  ...
  node.get_parameter("log_each_cmd_vel", log_each_cmd_vel_);
  ...
  if (should_send_command) {
    ...
    try {
      ...
      if (log_each_cmd_vel_) {
        ...
        RCUTILS_LOG_INFO(
            "[cmd_vel handle] seq=%llu latency=%.2fms inter_arrival=%.2fms x=%.4f yaw=%.4f "
            "m1_qpps=%d "
            "m2_qpps=%d dist=(%d,%d)",
...
void MotorDriver::controlLoopCallback() {
  ...
  auto* rc = roboclaw_;
  if (rc) {
    // Always read encoders every cycle for up-to-date odom + joint states
    // try {
    //   RoboClaw::EncodeResult encoder_result;
    //   CmdReadEncoder cmd1 = CmdReadEncoder(*rc, RoboClaw::kM1, encoder_result);
    //   cmd1.execute();
